...
public class TDView {
	...
	public TDStatus updateIndex() {
		...
		try {
			...
			if (lastSequence == 0) {
				...
				// doc in the output
				// We are finding the latest sequence and then finding the last
				// few sequences (except this one) for the same doc and removing
			...
			AbstractTouchMapEmitBlock emitBlock = new AbstractTouchMapEmitBlock() {
				...
				public void emit(Object key, Object value) {
					...
					try {
						String keyJson = TDServer.getObjectMapper()
								.writeValueAsString(key);
						String valueJson = TDServer.getObjectMapper()
	...
	public Cursor resultSetWithOptions(TDQueryOptions options, TDStatus status) {
		...
		// OPT: It would be faster to use separate tables for raw-or
		// ascii-collated views so that
		// they could be indexed with the right collation, instead of having to
		// specify it here.
		String collationStr = "";
	...
	public static boolean groupTogether(Object key1, Object key2, int groupLevel) {
		if (groupLevel == 0 || !(key1 instanceof List)
				|| !(key2 instanceof List)) {
		...
		int end = Math.min(groupLevel,
				Math.min(key1List.size(), key2List.size()));
		for (int i = 0; i < end; ++i) {
	...
	// Returns the prefix of the key to use in the result row, at this
	// groupLevel
	@SuppressWarnings("unchecked")
	public static Object groupKey(Object key, int groupLevel) {
		if (groupLevel > 0 && (key instanceof List)
				&& (((List<Object>) key).size() > groupLevel)) {
	 ...
	 *            The options to use.
	 * @param status
	 *            An array of result rows -- each is a dictionary with "key" and
	 *            "value" keys, and possibly "id" and "doc".
	 */
	@SuppressWarnings("unchecked")
	public List<Map<String, Object>> queryWithOptions(TDQueryOptions options,
			TDStatus status) {
		if (options == null) {
			options = new TDQueryOptions();
		}
		...
		try {
			...
			while (!cursor.isAfterLast()) {
				...
				if (reduce) {
					...
					if (group && !groupTogether(key, lastKey, groupLevel)
							&& (lastKey != null)) {
						// This pair starts a new group, so reduce & record the
						// last one:
						Object reduced = (reduceBlock != null) ? reduceBlock
					...
					if (options.isIncludeDocs()) {
						docContents = db.documentPropertiesFromJSON(
								cursor.getBlob(4), docId, cursor.getString(3),
								cursor.getLong(5), options.getContentOptions());
			...
			if (reduce) {
				if (keysToReduce.size() > 0) {
					// Finish the last group (or the entire list, if no
					// grouping):
					Object key = group ? groupKey(lastKey, groupLevel) : null;
					Object reduced = (reduceBlock != null) ? reduceBlock
							.reduce(keysToReduce, valuesToReduce, false) : null;
					Map<String, Object> row = new HashMap<String, Object>();
					row.put("key", key);
					if (reduced != null) {
						row.put("value", reduced);
		...
		for (Object object : values) {
			if (object instanceof Number) {
				...
				Log.w(TDDatabase.TAG,
						"Warning non-numeric value found in totalValues: "
								+ object);
			}
		}
		return total;
